export function printWeather(data) {
  ...
  document.querySelector(
    '.inform__pressure'
  ).innerHTML = `Pressure: ${data.pressure} MB`;
  document.querySelector(
    '.inform__feelslike'
  ).innerHTML = `Feels like: ${data.feelslike} &#8451;`;
  document.querySelector(
    '.inform__weather'
  ).innerHTML = `Today is ${data.discription}`;
  document.querySelector('.wind-direction').innerHTML = `Wind: ${data.windDir}`;
  document.querySelector(
    '.wind-speed'
  ).innerHTML = `Speed: ${data.windSpeed} km/h`;
}

export function printNotification(title, text, num) {
